...
export interface InfraestruturaJavaSwingOpcoes {
    /**
     * Comando de inicialização do host Swing.
     * Exemplo: `java`
     */
    comando?: string;
     ...
     * Exemplo: ['-jar', 'delegua-interface-grafica-swing-host.jar']
     */
    argumentos?: string[];
...
/**
 * Infraestrutura de interface gráfica baseada em host Java Swing em processo externo.
 *
 * O host deve falar JSON line-delimited por stdin/stdout.
 */
export class InfraestruturaJavaSwing implements InfraestruturaGraficaInterface {
    ...
    constructor(opcoes: InfraestruturaJavaSwingOpcoes = {}) {
        const opcoesProcesso: ProcessoExternoOpcoes = {
            comando: opcoes.comando ?? 'java',
            argumentos: opcoes.argumentos ?? ['-jar', 'delegua-interface-grafica-swing-host.jar'],
        ...
        this.processo.conectar('erro', (mensagem) => {
            ...
            console.error(`[InfraestruturaJavaSwing]${detalhe}`);
        });
    }
